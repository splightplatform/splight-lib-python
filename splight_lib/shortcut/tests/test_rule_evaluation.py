...
from splight_models import RuleVariable, AlgorithmRule
from splight_lib.shortcut import rule_eval
...
class TestRuleEval(TestCase):
    ...
    def test_rule_evaluation_false(self):
        self.assertFalse(rule_eval(rule=self.rule, db_client=self.database, dl_client=self.datalake))
    ...
    def test_unexistent_rule_evaluation(self):
        self.assertIsNone(rule_eval(rule=None, db_client=self.database, dl_client=self.datalake))