...
class TestPusherClient(TestCase):
    ...
    def test_init_with_args(self, _):
        client = PusherClient(self.channel)
        self.assertEqual(client._channel, self.channel)
    ...
    def test_client_authenticate(self, mocked_call, _):
        client = PusherClient(self.channel)
        client.authenticate(self.socket)
        mocked_call.assert_called_with(self.channel, self.socket)

    @patch.object(Pusher, '__init__', return_value=None)
    @patch('splight_notification.pusher.Pusher.trigger')
    def test_client_send(self, mocked_call, _):
        client = PusherClient(self.channel)
        client.send(self.topic, self.data)
        mocked_call.assert_called_with(self.channel, self.topic, self.data)
