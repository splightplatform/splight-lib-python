...
log_tags = {key: key.upper() for key in ["abstract_cache"]}
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Callable:
            ...
            try:
                ...
                logger.error('Error getting cache for %s', final_cache_key, tags=log_tags["abstract_cache"])

            if isinstance(cached_data, bytes):
                logger.info('Cache hit for %s', final_cache_key, tags=log_tags["abstract_cache"])
                ...
                try:
                    ...
                    logger.info('Cache update for %s', final_cache_key, tags=log_tags["abstract_cache"])
                except Exception:
                    logger.error('Error getting cache for %s', final_cache_key, tags=log_tags["abstract_cache"])
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Callable:
            ...
            try:
                ...
                logger.info('Cache flush for %s', final_cache_key, tags=log_tags['abstract_cache'])
            except Exception:
                logger.error('Error flushing cache for %s', final_cache_key, tags=log_tags["abstract_cache"])
